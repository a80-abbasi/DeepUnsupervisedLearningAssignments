...
def visualize_q2a_data(dset_type, local=False):
    data_dir = get_data_dir(1, local=local)
    if dset_type == 1:
        n, d = 10000, 25
...
def visualize_q2b_data(dset_type, local=False):
    data_dir = get_data_dir(1, local=local)
    if dset_type == 1:
...
def q2_save_results(dset_type, part, fn, local=False):
    data_dir = get_data_dir(1, local=local)
    if part == 'a':
        if dset_type == 1:
            n, d = 10000, 25
...
# Question 3
def q3a_save_results(dset_type, q3_a, local=False):
    data_dir = get_data_dir(1, local=local)
    if dset_type == 1:
        train_data, test_data = load_pickled_data(join(data_dir, 'shapes.pkl'))
        img_shape = (20, 20)
...
def q3bc_save_results(dset_type, part, fn, local):
    data_dir = get_data_dir(1, local=local)
    if dset_type == 1:
...
def visualize_q3b_data(dset_type, local=False):
    data_dir = get_data_dir(1, local=local)
    if dset_type == 1:
        train_data, test_data = load_pickled_data(join(data_dir, 'shapes_colored.pkl'))
        name = 'Colored Shape'
...
def q3d_save_results(dset_type, q3_d, local=False):
    data_dir = get_data_dir(1, local=local)
    if dset_type == 1:
...
# Question 4
def q4a_save_results(q4_a, local=False):
    data_dir = get_data_dir(1, local=local)
    train_data, test_data = load_pickled_data(join(data_dir, 'mnist_colored.pkl'))
    img_shape = (28, 28, 3)
...
def q4b_save_results(q4_b, local=False):
    data_dir = get_data_dir(1, local=local)
    train_data, test_data = load_pickled_data(join(data_dir, 'mnist_colored.pkl'))
    img_shape = (28, 28, 3)
...
def q4c_save_results(q4_c, local=False):
    data_dir = get_data_dir(1, local=local)
    train_data, test_data = load_pickled_data(join(data_dir, 'mnist.pkl'))
